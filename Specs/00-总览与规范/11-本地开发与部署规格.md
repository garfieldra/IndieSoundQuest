...
让开发者在取得必要模型 Key 后，通过一次宿主机 Java 打包和 Docker Compose 启动完整本地环境；确保服务依赖、环境变量、健康检查、数据库迁移和测试入口可复现。
...
- 未设置 `DEEPSEEK_API_KEY` 时，歌曲世界杯核心与基础业务仍可启动；Agent 策展/赛后引导返回明确的模型不可用状态。
...
1. 宿主机 Maven 打包 Java JAR；
2. MySQL、Redis、etcd、MinIO 与 Milvus 健康；
3. Java 执行数据库迁移并通过 `/actuator/health`；
4. Agent `/health/ready` 成功访问 Java 内部接口和必要配置；
5. Web 启动并可通过 Java 访问 API。
...
`depends_on` 必须使用健康检查条件，不能仅依赖容器启动顺序。Agent 的 Milvus 依赖可降级；Milvus 不就绪不应阻止歌曲世界杯核心启动。
...
- [ ] 宿主机 Maven 打包后，`docker compose up --build` 可启动默认完整系统；
