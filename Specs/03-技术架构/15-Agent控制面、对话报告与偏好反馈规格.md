...
当用户在同一轮明确列出多位艺人时，`search_web` 仍由 ReAct Supervisor 自主决定是否调用，但执行层必须将这些字面实体展开成相互独立的并发检索切面，并以轮询方式均衡合并各查询的来源，避免首个查询或单一艺人的结果占满证据窗口。`recommend_music` 必须观察明确艺人的覆盖情况；若模型草稿或 MusicBrainz 核验后结果异常集中于单一艺人，应先进行有界重试，并可从已高置信解析的明确艺人 MusicBrainz 目录中补充规范作品。最终七首展示结果优先保证跨艺人覆盖，再展示同一艺人的第二首作品。安溥/张悬等同一音乐人的阶段性署名可以合并解释，不要求伪造为两个不同实体。
