...
public class MusicCatalogInternalController {
  ...
  @GetMapping("/tournaments/{tournamentId}/report-facts") @org.springframework.transaction.annotation.Transactional(readOnly=true) Map<String,Object> reportFacts(@RequestHeader("Authorization") String authorization,@RequestHeader("X-Guest-Session-Id") UUID guestSessionId,@PathVariable UUID tournamentId){
    ...
    var matchFacts=matches.findByTournamentIdOrderByRoundNumberAscMatchIndexAsc(tournamentId).stream().filter(match->match.getStatus()==MatchStatus.COMPLETED).map(match->Map.<String,Object>of("matchId",match.getId(),"roundNumber",match.getRoundNumber(),"leftEntryId",match.getLeftEntryId(),"rightEntryId",match.getRightEntryId(),"winnerEntryId",match.getWinnerEntryId())).toList();
