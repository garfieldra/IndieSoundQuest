...
public class AgentWorkerInternalController {
  ...
  @PostMapping("/{id}/complete") void complete(@PathVariable UUID id,@RequestHeader("Authorization")String auth,@RequestHeader("X-Lease-Token")String lease,@RequestBody Completion body){authorize(auth);runs.assertLease(id,lease);var card=body.card()==null?null:new ConversationAgentGateway.Card(body.card().messageType(),body.card().cardType(),write(body.card().payload()));var result=new ConversationAgentGateway.Result(body.text(),card,body.memorySummary(),body.memorySummaryThroughSequence());conversations.complete(body.conversationId(),id,result);runs.complete(id,body.card()==null?"{}":write(body.card().payload()));runs.finishLease(id,lease);}
  @PostMapping("/{id}/waiting") void waiting(@PathVariable UUID id,@RequestHeader("Authorization")String auth,@RequestHeader("X-Lease-Token")String lease,@RequestBody Waiting body){authorize(auth);runs.assertLease(id,lease);var card=new ConversationAgentGateway.Card(body.card().messageType(),body.card().cardType(),write(body.card().payload()));conversations.completeWaitingForUser(body.conversationId(),id,new ConversationAgentGateway.Result(body.text(),card,null,null),body.snapshot());runs.finishLease(id,lease);}
