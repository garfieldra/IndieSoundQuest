...
type RunEvent = { sequenceNumber: number; type: string; payloadJson: string; createdAt: string }
...
async function createAndWaitForReport(tournamentId: string): Promise<Report> {
  ...
  for (let attempt = 0; attempt < 330 && report.status !== 'READY'; attempt++) {
    ...
    if (report.runId) {
      const snapshot = await api<{ runStatus: string; events: RunEvent[] }>(`/agent-runs/${report.runId}/events?afterSequence=${cursor}`)
      for (const event of snapshot.events) {
        cursor = Math.max(cursor, event.sequenceNumber)
        if (event.type === 'FAILED') throw new Error('报告生成失败，请稍后重试。')
      }
    }
...
async function queuedRun<T>(path:string,options:RequestInit,onProgress:(item:Progress)=>void,onPlan:(item:Plan)=>void):Promise<T>{const queued=await api<{runId:string}>(path,options);let cursor=0;for(let attempt=0;attempt<900;attempt++){const snapshot=await api<{runStatus:string;events:RunEvent[]}>(`/agent-runs/${queued.runId}/events?afterSequence=${cursor}`);for(const event of snapshot.events){cursor=Math.max(cursor,event.sequenceNumber);const data=JSON.parse(event.payloadJson) as Record<string,unknown>;if(event.type==='PROGRESS'||event.type==='RETRY')onProgress({phase:String(data.phase||'progress'),message:String(data.message||''),elapsedMs:Number(data.elapsedMs||0)});if(event.type==='PLAN_UPDATED')onPlan(data as unknown as Plan);if(event.type==='RESULT')return data as unknown as T;if(event.type==='FAILED')throw new Error(String(data.message||'Agent 任务失败'))}if(snapshot.runStatus==='FAILED')throw new Error('Agent 任务失败');await new Promise(resolve=>window.setTimeout(resolve,1000))}throw new Error('Agent 任务等待超时')}
...
export function ConversationApp() {
  ...
  async function replayRunEvents(runId: string) {
    try {
      ...
      for (const event of payload.events) {
        ...
        if (event.type === 'PROGRESS') replayProgress.push({ phase: String(data.phase || 'progress'), message: String(data.message || ''), elapsedMs: Number(data.elapsedMs || 0) })
      ...
      if (payload.runStatus === 'RUNNING' || payload.runStatus === 'WAITING_FOR_USER') setCollapsed(false)
  ...
  async function send(event: FormEvent) {
    ...
    try {
      const queued = await api<{ runId: string; status: string }>(`/conversations/${current.id}/messages`, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': clientId }, body: JSON.stringify({ content }) })
      let cursor = 0; let terminal = false
      for (let attempt = 0; attempt < 900 && !terminal; attempt++) {
        const snapshot = await api<{ runStatus: string; events: RunEvent[] }>(`/agent-runs/${queued.runId}/events?afterSequence=${cursor}`)
        for (const item of snapshot.events) {
          cursor = Math.max(cursor, item.sequenceNumber)
          const data = JSON.parse(item.payloadJson) as Record<string, unknown>
          if (item.type === 'PROGRESS') setProgress(previous => [...previous, { phase: String(data.phase || 'progress'), message: String(data.message || ''), elapsedMs: Number(data.elapsedMs || 0) }])
          if (item.type === 'PLAN_UPDATED') setPlan(data as unknown as Plan)
          if (item.type === 'FAILED') throw new Error(String(data.message || '这次音乐对话暂时无法完成'))
        }
        terminal = ['COMPLETED', 'WAITING_FOR_USER'].includes(snapshot.runStatus)
        if (!terminal) await new Promise(resolve => window.setTimeout(resolve, 1000))
      }
      if (!terminal) throw new Error('Agent 任务等待超时')
  ...
  async function requestExplorationReport() {
    ...
    try {
      const response = await fetch(`/api/v1/conversations/${current.id}/exploration-report`, { method: 'POST', credentials: 'include', headers: { 'Content-Type': 'application/json', 'Idempotency-Key': crypto.randomUUID() } })
      if (!response.ok || !response.body) throw new Error(`请求失败（${response.status}）`)
      const reader = response.body.getReader(); const decoder = new TextDecoder(); let buffer = ''; let completed = false
      const consume = (block: string) => { const type = block.match(/^event:\s*(.+)$/m)?.[1]?.trim(); const data = block.match(/^data:\s*(.+)$/m)?.[1]?.trim(); if (!type || !data) return; if (type === 'progress') setProgress(previous => [...previous, JSON.parse(data) as Progress]); else if (type === 'plan_updated') setPlan(JSON.parse(data) as Plan); else if (type === 'message_completed') completed = true; else if (type === 'error') throw new Error((JSON.parse(data) as { message?: string }).message || '探索报告暂时无法生成') }
      ...
      if (!completed) throw new Error('探索报告未返回最终结果')
      await refreshMessages(); const refreshed = await api<Conversation[]>('/conversations'); setConversations(refreshed); setCurrent(refreshed.find(item => item.id === current.id) ?? current)
  ...
  return <main className="conversation-shell">
    ...
    <section className="conversation-main"><header className="conversation-header"><p className="eyebrow">音乐探索对话</p><h1>{current?.title || '新的音乐探索'}</h1><p>从一句偏好开始，候选、对局和报告都会留在这段对话里。</p><div><button type="button" disabled={!current || pending || Boolean(candidateRun)} onClick={() => void requestExplorationReport()}>整理探索报告</button></div></header>{notice && <p className="conversation-notice">{notice}</p>}
