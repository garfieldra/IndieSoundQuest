...
export function ConversationApp() {
  ...
  return <main className="conversation-shell">
    ...
    <section className="conversation-main"><header className="conversation-header"><p className="eyebrow">音乐探索对话</p><h1>{current?.title || '新的音乐探索'}</h1><p>从一句偏好开始，问答、推荐、资料线索和探索报告都会留在这段对话里。</p><div><button type="button" disabled={!current || pending || Boolean(candidateRun)} onClick={() => void requestExplorationReport()}>整理探索报告</button></div></header>{notice && <p className="conversation-notice">{notice}</p>}
      ...
      <div className="conversation-agent-inline">{(pending || candidateRun || plan || progress.length > 0) && <AgentRun plan={plan} progress={progress} collapsed={collapsed} active={pending || Boolean(candidateRun)} onToggle={() => setCollapsed(value => !value)} />}</div>
...
function AgentRun({ plan, progress, collapsed, active, onToggle }: { plan: Plan | null; progress: Progress[]; collapsed: boolean; active: boolean; onToggle: () => void }) { const latest = progress[progress.length - 1]; return <aside className={`conversation-run ${collapsed ? 'collapsed' : ''}`}><button onClick={onToggle}><span>{active ? '正在整理这次音乐探索' : '已完成本次音乐探索'}</span><small>{latest?.message || plan?.summary}</small><span>{collapsed ? '展开' : '收起'}</span></button>{!collapsed && <>{plan && <><small className="conversation-plan-change">{plan.changeSummary}</small><ol className="conversation-plan">{plan.items.map(item => <li key={item.id} className={item.status}><span>{item.title}</span><small>{item.detail}</small></li>)}</ol></>}{progress.length > 0 && <ol className="conversation-progress">{progress.map((item, index) => <li key={`${item.phase}-${index}`} className={item.status}><span>{item.message}</span>{progressMetricsText(item) && <small>{progressMetricsText(item)}</small>}</li>)}</ol>}</>}</aside> }
...
function MusicRecommendationsCard({ payload, conversationId }: { payload?: string | null; conversationId?: string }) {
  ...
  return <section className="tournament-launch-card conversation-report-card music-recommendation-card"><p className="eyebrow">{value.contextMode === 'REFINED' ? '音乐推荐 · 基于上一轮调整' : '音乐推荐'}</p><h2>{value.title || '这轮值得继续听的方向'}</h2>{value.appliedInstruction && <small className="recommendation-refinement">已应用你的调整：{value.appliedInstruction}</small>}{value.summary && <p>{value.summary}</p>}{value.songs?.length ? <><h3>歌曲</h3><ol className="music-recommendation-list">{value.songs.map((item, index) => { const key=`song-${item.recordingId || index}`; return <li key={key}>{item.searchUrl ? <a className="music-recommendation-main" href={item.searchUrl} target="_blank" rel="noreferrer">{item.coverUrl ? <img src={item.coverUrl} alt="" /> : <span className="music-cover-placeholder">ISQ</span>}<span><strong>{item.title}</strong><small>{item.artistName}{item.albumTitle ? ` · ${item.albumTitle}` : ''}</small><em>{item.reason}</em></span></a> : <span className="music-recommendation-main"><span className="music-cover-placeholder">ISQ</span><span><strong>{item.title}</strong><small>{item.artistName}</small><em>{item.reason}</em></span></span>}<div className="music-recommendation-actions"><small>{item.verificationStatus === 'MUSICBRAINZ_VERIFIED' ? 'MusicBrainz 已核验' : '公开资料发现'}</small><button onClick={() => void sendFeedback('SONG', key, { recordingId: item.recordingId, title: item.title, artistName: item.artistName }, 'LIKE')}>喜欢</button><button onClick={() => void sendFeedback('SONG', key, { recordingId: item.recordingId, title: item.title, artistName: item.artistName }, 'DISLIKE')}>不适合</button>{feedback[key] && <small>已记录</small>}</div></li> })}</ol></> : null}{value.artists?.length ? <><h3>艺人</h3><ol className="music-artist-list">{value.artists.map((item, index) => { const key=`artist-${item.artistId || item.artistName || index}`; return <li key={key}><a href={item.searchUrl || item.sourceUrl} target="_blank" rel="noreferrer"><strong>{item.artistName}</strong><small>{item.reason}</small></a><div className="music-recommendation-actions"><small>{item.verificationStatus === 'MUSICBRAINZ_VERIFIED' ? 'MusicBrainz 已核验' : '公开资料发现'}</small><button onClick={() => void sendFeedback('ARTIST', key, { artistId: item.artistId, artistName: item.artistName }, 'LIKE')}>喜欢</button><button onClick={() => void sendFeedback('ARTIST', key, { artistId: item.artistId, artistName: item.artistName }, 'DISLIKE')}>不适合</button>{feedback[key] && <small>已记录</small>}</div></li> })}</ol></> : null}{value.sources?.length ? <details className="music-recommendation-sources"><summary>查看推荐依据</summary>{value.sources.map((item, index) => <a key={`${item.url}-${index}`} href={item.url} target="_blank" rel="noreferrer">{item.title || '公开资料'}</a>)}</details> : null}</section>
}
